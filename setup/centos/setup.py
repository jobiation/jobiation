...
if installansible.lower() == "y":
    # Install Ansible
    subprocess.call("./ansible_install.sh '"+current_user+"'",shell=True);
...
if installsmb.lower() == "y":
    ...
    # Add Samba firewall rules
    subprocess.call("./samba_firewall.sh");

    # Add SELinux permissions
    subprocess.call("./samba_selinux.sh '"+current_user+"'",shell=True);
...
if installvault.lower() == "y":
    ...
    bashfile.write("su $1 -c '/home/" + current_user + "/.local/bin/ansible-vault encrypt_string --encrypt-vault-id " + vault_username + " --vault-id " + vault_username + "@prompt \"" + device_password + "\" --name \"ansible_password\" > encpassword.txt'\n");
    bashfile.write("echo 'Enter your Ansible Vault password twice to encrypt your ansible_username';\n");
    bashfile.write("su $1 -c '/home/" + current_user + "/.local/bin/ansible-vault encrypt_string --encrypt-vault-id " + vault_username + " --vault-id " + vault_username + "@prompt \"" + device_username + "\" --name \"ansible_user\" > encusername.txt'\n");
    bashfile.write("echo 'Enter your Ansible Vault password twice to encrypt your ansible_become_password (AKA enable password)';\n");
    bashfile.write("su $1 -c '/home/" + current_user + "/.local/bin/ansible-vault encrypt_string --encrypt-vault-id " + vault_username + " --vault-id " + vault_username + "@prompt \"" + device_become + "\" --name \"ansible_become_password\" > encbecome.txt'");
    bashfile.close();
    os.chmod("runvaultcmd.sh", 0o770);
    subprocess.call("./runvaultcmd.sh '"+current_user+"'",shell=True);
